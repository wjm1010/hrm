...
<script>
  ...
  import { getUserInfoByCallPhone } from '@/utils/getInfoMethods.js'
  ...
  export default {
    ...
    methods: {
      ...
      getTeacherIsHaveRole(studentIntentionId) {
        this.$http.get('teacher_isHaveRole', {
          params: {
            studentIntentionId
          }, isFilterUrl: true
        }).then(res => {
          if (res.data) {
            this.$router.push(`/roster/detail/${studentIntentionId}`)
          } else {
        ...
        this.callSetTimeOutId = setTimeout(async() => {
          try {
            ...
            if (this.isHangUpPhone(status)) {
              ...
              userinfo.unFinishCallId && this.$router.push(`/roster/detail/${userinfo.unFinishCallStudentIntentionId}`) // 防止未挂断已添加沟通记录情况
              return
            }
            this.startCallSetTimeOut()
          } catch (e) {
        ...
        if (!this.callPhoneMessage[status]) {
          ...
          switch (status) {
            ...
            case '2':
              ...
              userinfo.unFinishCallId && this.$route.name === 'rosterdetail' && this.$message.warning('通话结束，请添加沟通记录！')
              return true
          }
        }
        return false
