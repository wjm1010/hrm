  ...
  <el-row :gutter="20">
    ...
    <el-col :span="24">
      <expand-table
        ...
        <template slot-scope="scope" slot="activeOptions">
          <el-popover trigger="hover" placement="top" v-if="scope.row['activeOptions'].length>10" width="500">
            <p>{{scope.row['activeOptions']}}</p>
            <div slot="reference" class="name-wrapper">
              <p>{{scope.row['activeOptions'].slice(0,10)}}...</p>
            </div>
          </el-popover>
          <p v-else>{{scope.row['activeOptions']}}</p>
        </template>
  ...
  data() {
    return {
      ...
      dialogRules: {
        ...
        activeOption: [
          { required: true, trigger: 'change', message: '请选择激活项！' }
        ],
        remarks: [
...
@import '../../styles/closed/index.sass'
