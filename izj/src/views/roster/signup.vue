  ...
  <div class="dashboard-container">
    ...
    <expand-table
      ...
      <template slot-scope="scope" slot="operating">
        <span v-if="scope.row.is_apply === '1'">
          已报名
        </span>
    ...
    <el-dialog
      ...
      <div class="content">
        <span>是否报名“</span>
        <b class="color-blue">{{dialog.signUp.data.version_name}}</b>
        <span>”课程</span>
      </div>
      <div slot="footer">
        <el-button
    ...
    data() {
      return {
        ...
        columns: [
          ...
          {
            value: 'version_name', text: '版本名称'
          },
          {
            value: 'curr_plan_name', text: '课程名称'
          },
          {
    ...
    methods: {
      ...
      postCourseAddApplyCourse() {
        return this.$http.post('course_addApplyCourse', {
          ...
          currPlanPropertyId: this.dialog.signUp.data.curr_plan_property_id
        })
      },
      getList() {
        ...
        this.getCourseApplyCourse(_query).then(res => {
          if (res.data) {
