<template>
  <div class="dashboard-container">
    <div v-loading="allLoading">
      <div v-if="!strategyId || !allLoading">
        ...
        <order-form
          ...
          v-if="publicQuery.isSort === '2'"
        >
        </order-form>

        <template v-if="publicQuery.isSort === '2'">
          <el-form
    ...
    <el-dialog
      ...
      <span>当前存在同一学科拓科试听课未上，是否取消未上拓科试听课？</span>
      <span slot="footer" class="dialog-footer">
      <el-button @click="isShowDialogVisible = false">取 消</el-button>
      <el-button type="primary" @click="submitDialogAndGoView">取消课程</el-button>
    </span>
    </el-dialog>
    ...
    <bad-lesson-dialog
      ...
      :studentNo="studentNo"
      @submit-event="submitAndGoView('2')">
    </bad-lesson-dialog>
  ...
  export default {
    ...
    data() {
      return {
        ...
        /* query */
        strategyId: this.$route.path !== '/expandLesson/add' ? this.$route.params.strategyId : '', // 编辑传id
        ...
        /* query */
        baseInfoQuery: {
          contractType: this.$route.path !== '/expandLesson/add' ? '' : '1',
          contractUsable: ''
        },
    ...
    methods: {
      ...
      init() {
        this.getStrategiesDetails().then(res => {
          if (res.data) {
            ...
            const startDate = audition_start_time.split(' ')
            const endDate = audition_end_time.split(' ')
            this.publicQuery.date = startDate[0]
            this.publicQuery.tryStartTime = startDate[1]
            this.publicQuery.tryEndTime = endDate[1]
      ...
      orderFormSubmit() {
        ...
        const { date, tryStartTime, tryEndTime } = this.publicQuery
        postBody.auditionStartTime = `${date} ${tryStartTime}`
        postBody.auditionEndTime = `${date} ${tryEndTime}`
        delete postBody.lessonTime
        delete postBody.platoonTimeArrObj
        delete postBody.date
      ...
      async postSubmit(postBody) {
        ...
        try {
          ...
          // 保存
          if (this.isSaveSubmit) return this.goDetailView()
      ...
      goDetailView() {
        this.newIsSort === '2' ? this.$router.replace({
          ...
          query: { strategyId: this.newStrategyId, type: '2' }
        }) : this.$router.replace({
      ...
      submitDialogAndGoView() {
        this.isShowDialogVisible = false
        this.submitAndGoView('3')
      },
      // 提交
      async submitAndGoView(submitType) {
        this.isBadLessonDialogShow = false
        const { data, code, message } = await this.postSubmitAPi(submitType)
        if (code === '0x000002') {
