<template>
  <div class="dashboard-container">
    ...
    <div class="fr">
      <el-button type="primary" @click="add">新增数据员</el-button>
    </div>
...
export default {
  ...
  methods: {
    getOptions() {
      this.$http.post('auth_getDataerOption').then(res => {
        if (res.data) {
    ...
    getList() {
      ...
      this.loading = true
      this.$http.post('dataMember_getUserList', this.query, { paging: this.paging }).then(res => {
        if (res.data) {
    ...
    edit(row) {
      this.$router.push(`/staff/dataOfficer/edit/${row.userId}`)
    },
    add() {
      this.$router.push('/staff/dataOfficer/add')
    }
  }
}
</script>
