...
export default {
  ...
  data() {
    return {
      isCancearch: false,
      userId: this.$route.path !== '/staff/admin/add' ? this.$route.params.userId : '',
      allLoading: false,
      query: {
        userName: '',
        userPwd: 'hfjy123',
  ...
  methods: {
    // api
    getInfo() {
      return this.$http.post('systemMan_dataListOne', {
        userId: this.userId
      })
    },
    getOptions() {
      return this.$http.post('auth_getAdminOption')
    },
    async init() {
      const option = await this.getOptions()
      if (option.data) {
        this.options.status = option.data.status
        this.options.roleList = option.data.roleList[7]
        if (this.userId) {
          this.getEditInfo()
    ...
    removeRouter(formName) {
      this.$router.replace('/staff/admin')
      if (!this.userId) {
        this.$refs[formName].resetFields()
        return this.$store.dispatch('delVisitedViews', { name: 'adminIndexAdd', path: '/staff/admin/add' })
      }
    },
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.userId) {
            this.$http.post('systemMan_dataUpdateOne', Object.assign({ userId: this.userId }, this.query)).then(res => {
              if (res.data) {
            ...
            this.$http.post('systemMan_roleInsert', this.query).then(res => {
              if (res.data) {
                this.removeRouter(formName)
                this.$message.success(res.message)
              }
