...
<script>
export default {
  name: 'dataOfficerIndexAdd',
  data() {
    return {
      isCancearch: false,
      userId: this.$route.path !== '/staff/dataOfficer/add' ? this.$route.params.userId : '',
      allLoading: false,
      query: {
        userName: '',
        userPwd: 'hfjy123',
  ...
  methods: {
    // api
    getInfo() {
      return this.$http.post('dataMember_dataListOne', {
        userId: this.userId
      })
    },
    getOptions() {
      return this.$http.post('auth_getDataerOption')
    },
    async init() {
    ...
    removeRouter(formName) {
      this.$router.replace('/staff/dataOfficer')
      if (!this.userId) {
        this.$refs[formName].resetFields()
        return this.$store.dispatch('delVisitedViews', { name: 'dataOfficerIndexAdd', path: '/staff/dataOfficer/add' })
      }
    },
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.userId) {
            this.$http.post('dataMember_dataUpdateOne', this.query).then(res => {
              if (res.data) {
            ...
            this.$http.post('dataMember_roleInsert', this.query).then(res => {
              if (res.data) {
                this.removeRouter(formName)
                this.$message.success(res.message)
              }
