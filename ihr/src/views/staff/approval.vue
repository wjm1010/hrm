<template>
  <div class="dashboard-container">
    <el-row>
      <el-form label-width="120px" inline>
        ...
        <el-form-item label="教学部：">
          <el-select v-model="query.userEducationDept" clearable filterable placeholder="请选择">
            <el-option
    ...
    <div class="fr">
      <el-button type="primary" @click="add">新增审批官</el-button>
    </div>
...
export default {
  ...
  methods: {
    getOptions() {
      this.$http.post('auth_getApprovalOption').then(res => {
        if (res.data) {
    ...
    getList() {
      ...
      this.loading = true
      this.$http.post('approval_getUserList', this.query, { paging: this.paging }).then(res => {
        if (res.data) {
    ...
    edit(row) {
      this.$router.push(`/staff/approval/edit/${row.userId}`)
    },
    add() {
      this.$router.push('/staff/approval/add')
    }
  }
}
</script>
