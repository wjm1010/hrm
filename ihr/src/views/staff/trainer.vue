<template>
  <div class="dashboard-container">
    ...
    <div class="fr">
      <el-button type="primary" @click="add">新增培训师</el-button>
    </div>
...
export default {
  ...
  methods: {
    getOptions() {
      this.$http.post('auth_getTrainerOption').then(res => {
        if (res.data) {
          this.options.status = res.data.status
          this.options.roleList = res.data.roleList[8]
        }
      }).catch(console.log)
    },
    getList() {
      ...
      this.loading = true
      this.$http.post('trainer_getUserList', this.query, { paging: this.paging }).then(res => {
        if (res.data) {
    ...
    edit(row) {
      this.$router.push(`/staff/trainer/edit/${row.userId}`)
    },
    add() {
      this.$router.push('/staff/trainer/add')
    }
  }
}
</script>
