...
import { getToken, getRoleType } from '@/utils/auth' // getToken from cookie
    ...
    if (to.path === '/login') {
      ...
      if (store.getters.roles.length === 0) { // 判断当前用户是否已拉取完user_info信息
          ...
          store.dispatch('GenerateRoutes', { roles }).then(() => { // 根据roles权限生成可访问的路由表
            ...
            try {
              ...
              if (userinfo && userinfo.unFinishCallStudentIntentionId) {
                ...
                if (routeName) {
                  next({ path: `${routeName}${userinfo.unFinishCallStudentIntentionId}`, replace: true })
                } else {
        ...
        if (hasPermission(store.getters.roles, to.meta.roles)) {
          if (!to.path.includes('roster/detail') && !to.path.includes('student/detail')) {
            return next()
          }
