<template>
  ...
  <div class="main">
    <div>
      <div class="header">
        <span class="operating-name">快捷操作</span>
      </div>
      <ul class="left-list">
        <li
        ...
        >
          <div class="iconContent" v-if="item.icon">
            <i class="icon" :style="{ backgroundImage: `url(${item.icon})`}"></i>
          </div>
          <span v-if="item.icon">{{item.text}}</span>
          <span v-else class="knowledgeSearch">{{item.text}}</span>
        </li>
      </ul>
    </div>
    <centerDialog
...
<script>
  ...
  import classIcon from '../icons/icon-14.png'
  import passwordIcon from '../icons/icon-11.png'
  import plemIcon from '../icons/icon-12.png'
  import clientIcon from '../icons/icon-13.png'
  import reportIcon from '../icons/icon-15.png'
  import addTalkIcon from '../icons/icon-16.png'
  import focusIcon from '../icons/icon-17.png'
  import transferIcon from '../icons/icon-10.png'
  import applyIcon from '../icons/icon-01.png'
  import regerferIcon from '../icons/icon-02.png'
  ...
  export default {
    ...
    data() {
      return {
        leftList: [
          {
            icon: addTalkIcon, text: '添加沟通记录', clickEvent: this.communicationRecordBtnEvent
          },
          {
            icon: plemIcon, text: '补全资料', clickEvent: this.materialBtnEvent
          },
          {
            icon: clientIcon, text: '开通客户端', clickEvent: this.openClientBtnEvent
          },
          {
            icon: passwordIcon, text: '密码重置', clickEvent: this.resetPasswordBtnEvent
          },
          {
            icon: classIcon, text: '上体验课', clickEvent: this.experienceClassBtnEvent
          },
          {
            icon: applyIcon, text: '申请试听课', clickEvent: this.trialLessonBtnEvent
          },
          {
            icon: reportIcon, text: '报名', clickEvent: this.signUpBtnEvent
          },
          {
            icon: regerferIcon, text: '成单登记', clickEvent: this.singleRegistrationBtnEvent
          },
          {
            icon: transferIcon, text: '移交别人', clickEvent: this.transferStudentBtnEvent
          },
          {
            icon: focusIcon, text: '关注“海风教育”', clickEvent: this.attentionQrCode
          },
          {
            text: '知识库搜索', clickEvent: this.knowledgeSearch
          }
        ],
        listData: [],
    ...
    methods: {
      ...
      materialBtnEvent() {
        const judge = '1'
        this.$router.push(`/student/material/${this.rosterId}/${judge}`)
      },
...
<style lang="sass" scoped>
  @import '../../../styles/student/detail/operating'
</style>
