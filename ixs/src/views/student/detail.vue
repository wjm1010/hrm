    ...
    <template v-if="canGetStudentInfo">
      <template>
        <Info
          ...
          :indexInit="init"
          :roster-id="rosterId"
          :checkIdresult="dialog.communicationRecord.trackUserId === this.userId"
          :fixDialog="fixDialog"
        ...
        <el-menu
          ...
          <el-menu-item index="courseInfo">课程信息</el-menu-item>
          <el-menu-item index="studentCalendar">学生日历</el-menu-item>
          <el-menu-item index="studentTransfer">学生流转</el-menu-item>
        </el-menu>

        <div class="container">
          <DetailModules style="width: 87%" :modules="modulesOrder">
            <template v-if="activeTab === 'studentInfo'">
              <div class="studentInfo" slot="baseInfo">
                <BasicInfo
                  :roster-id="rosterId"
                  :indexInit="init"
                  :fixDialog="fixDialog"
            ...
            <template v-else-if="activeTab === 'courseInfo'">
              <CalendarTable
                :fixDialog="fixDialog"
                :roster-id="rosterId"
                :student-id="studentId"
            ...
            <template v-else-if="activeTab === 'studentCalendar'">
              <Calendar
                slot="calendar"
                :roster-id="rosterId"
                :student-id="studentId"
      ...
      <div callIdAttr="call-phone-dialog" :class="{'communication-class': true, 'no-sidebar-navbar-record': sidebar.opened}" v-if="fixDialog.parentRandom" @click="fixShowDialog">沟通记录</div>
      <div :class="{'retrieve-list': true, 'no-sidebar-navbar-dialog': sidebar.opened }" @click="dialogRetrieveList.show = true" v-if="isHaveRetrieveList">寻回名单</div>
    </template>
    <template v-if="!canGetStudentInfo">
...
<script>
  ...
  import RestClass from './detail/restTime.vue' // 剩余课时
  ...
  export default {
    ...
    data() {
      return {
        ...
        modulesOrder: ['baseInfo', 'communicationDetails', 'classTeacher', 'calendarTable', 'singleMessage', 'calendar', 'restClass', 'studentCirculation']
      }
    },
    computed: {
      ...
      ...mapGetters(['userId', 'callStatus', 'departmentId', 'roleId', 'sidebar'])
    },
    watch: {
    ...
    created() {
      ...
      this.$eventBus.$on('experience-jump', this.experienceClassEvent)
      this.$eventBus.$on(`${this.rosterId}callLocal`, this.callPhoneLocal)
    },
    destroyed() {
      this.$eventBus.$off('experience-jump')
      ...
      getStudentInfo() {
        this.studentInfoApi().then(res => {
          if (res.data) {
            ...
            this.dialogRetrieveList.messageBody = `确定将${this.studentInfo.name}（${this.studentInfo.student_no}）寻回吗？`
...
<style lang="sass" scoped>
  @import '../../styles/student/detail.sass'
  .communication-class.no-sidebar-navbar-record
    left: 38%
  .retrieve-list.no-sidebar-navbar-dialog
    left: 45%
