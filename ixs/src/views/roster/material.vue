<template>
    ...
    <div class="pos-margin">
      <el-button type="primary" plain @click="materialCancleEvent" v-if="judgeOC">取消</el-button>
          ...
          if (i === j) {
            ...
            this.postMaterialStudentData(filterCopyObj).then(res => {
              if (res.data) {
                ...
                this.$router.replace({
                  path: `/roster/material/${this.rosterId}/${this.$route.params.judge}`, query: {
                    random: +new Date()
                  }
                })
                return this.$store.dispatch('delVisitedViews', { name: 'rostermaterial', path: `/roster/material/${this.rosterId}/${this.$route.params.judge}` })
              } else {
                this.$message.error({
                  dangerouslyUseHTMLString: true,
