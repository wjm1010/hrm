  ...
  <div class="main">
    ...
    <expand-table
      ...
      <template slot-scope="scope" slot="downloadUrl">
        <el-popover trigger="hover" placement="top" v-if="scope.row['downloadUrl'] && scope.row['downloadUrl'].length>15" width="500">
          <span>{{scope.row['downloadUrl']}}</span>
          <div slot="reference" class="name-wrapper">
            <span>{{scope.row['downloadUrl'].slice(0,13)}}...</span>
          </div>
        </el-popover>
        <span v-else>{{scope.row['downloadUrl']}}</span>
      </template>
