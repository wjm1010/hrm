    ...
    <div class="main">
      <el-form label-position="right" label-width="120px" ref="form" :model="query" inline>
        ...
        <el-form-item prop="allot">
          <el-radio-group v-model="query.allot" >
            <el-radio v-for="(item,index) in options.allot" :key="index" :label="item.key">{{item.value}}
            </el-radio>
          </el-radio-group>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // list
      postListData(query) {
        return this.$http.post('follow_newNameList', query, { paging: this.paging })
      },
      getList() {
        this.loading = true
...
@import '../../styles/tasks/newTasks.sass'
</style>
