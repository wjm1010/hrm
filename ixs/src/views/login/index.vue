  ...
  <div class="login-container">
    ...
    <el-form class="login-form" autoComplete="on" :model="loginForm" :rules="loginRules" ref="loginForm" label-position="left">
      ...
      <el-form-item prop="password">
        ...
        <el-input name="password" :type="passwordType" minlength="6" @keyup.enter.native="passwordLogin" v-model.trim="loginForm.password" autoComplete="on" placeholder="密码" @change="changeInput"/>
      </el-form-item>
      <div class="login-pwd">
      ...
      <el-button type="primary" size="medium" class="username" :loading="loading" @click.native.prevent="passwordLogin">{{$t('login.logIn')}}</el-button>
      <!-- <el-button style="display: none" type="info" class="dingding" @click.native.prevent="dingdingLogin">钉钉扫码登录</el-button> -->
    </el-form>

    <DingDialog
     v-if="dingDialog.show"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      doLogin(data) {
        data.redirectUrl = encodeURI(window.location.origin)
        data.redirectType = 'json'
        this.$store.dispatch('SSOLogin', data).then(res => {
          if (res.data) {
            this.dingDialog.show = true
            this.dingDialog.url = res.data.url
          }
          this.loading = false
    ...
    mounted() {
      ...
      if (url.has('token')) {
        this.$store.dispatch('SSOUser', url.get('token')).then(res => {
          window.location.href = '/'
        })
      } else {
        const message = url.get('message') || url.get('errmsg')
        if (message) {
