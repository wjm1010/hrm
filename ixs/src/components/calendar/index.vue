      ...
      <div class="calendar-info" @click.stop="changeYear">
        ...
        <div class="month">
          <div class="month-inner" :style="{'top':-(this.month*20)+'px'}">
            <span :key="key" v-for="(m, key) in months">{{m}}</span>
          </div>
        </div>
        ...
        <td class="td-day" :key="k2" v-for="(child,k2) in day"
          ...
          <div class="text item" v-if="child.eventName!==undefined">
            <div v-if="key<=2" :key="key" v-for="(item, key) in child.eventName">
              <div v-if="item.lessonType === '2'" class="text-align-left">
                <i class="trial-lesson"></i>
                <span class="color-aaa">{{item.lessonPlanTime + ' '+ item.teacherName + ' ' +item.lessonName}}</span>
              </div>
              <div v-if="item.lessonType === '1'" class="text-align-left">
                <i class="formal-lesson"></i>
                <span class="color-aaa">{{item.lessonPlanTime + ' '+ item.teacherName + ' ' +item.lessonName}}</span>
              </div>
            </div>
            <el-popover
              ...
              trigger="hover"
            >
              <div :key="key" v-for="(item, key) in child.eventName">
                <div v-if="item.lessonType === '2'" class="text-align-left">
                  <i class="trial-lesson"></i>
                  <span class="color-aaa">{{item.lessonPlanTime + ' '+ item.teacherName + ' ' +item.lessonName}}</span>
                </div>
              ...
              <div class="hover-text" v-if="child.eventName.length>2" slot="reference">......</div>
            </el-popover>
          </div>
        </td>
      </tr>
